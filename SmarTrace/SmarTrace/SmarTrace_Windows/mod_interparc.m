function [pt,dudt,fofthandle] = mod_interparc(l,px,py,varargin)
...
% any of the methods.
chordlen_tot = sqrt(sum(diff(pxy,[],1).^2,2));

% Normalize the arclengths to a unit total
chordlen = chordlen_tot/sum(chordlen_tot);
...
if method(1) == 'l'
  % The linear method.
  % pktest error('linear is not supported')
...
%   tbins((tbins <= 0) | (t <= 0)) = NaN;
%   tbins((tbins >= n) | (t >= 1)) = NaN;

  totalsplinelength = sum(chordlen_tot);
  cumarc = [0;cumsum(chordlen_tot)];
  [junk,tbins] = histc(l,cumarc); %#ok
  
  tbins((tbins <= 0) | (l <= 0)) = NaN;
  tbins((tbins >= n) | (l >= totalsplinelength)) = NaN;
  
  idx = find(~isnan(tbins));
  tbins = tbins(idx);
  t = l(idx)';

  % interpolate
  s = (t - cumarc(tbins))./chordlen_tot(tbins);
...
% NRtest [junk,tbins] = histc(t*totalsplinelength,cumseglen); %#ok
[junk,tbins] = histc(l,cumseglen); %#ok

% catch any problems at the ends
% NRtest tbins((tbins <= 0) | (t <= 0)) = 1;
% NRtest tbins((tbins >= n) | (t >= 1)) = n - 1;

% NRtest
%tbins((tbins <= 0) | (l <= 0)) = 1;
%tbins((tbins >= n) | (l >= totalsplinelength)) = n - 1;
...
for i = 1:nt
  ...
  % NRtest [tout,yout,te,ye] = ode45(@(t,y) segkernel(t,y),[0,chordlen(tbins(i))],-si,opts); %#ok
...
% NRtest remove NaNs at the end
firstnan = find(isnan(pt),1);
if firstnan
    pt = pt(1:firstnan-1,:);
end
